(function (global, factory) {
  ...
  const enableClientRenderFallbackOnTextMismatch = true; // TODO: Need to review this code one more time before landing
  ...
  const SVG_NAMESPACE = 'http://www.w3.org/2000/svg'; // Assumes there is no parent namespace.
  ...
  const PassiveMask = Passive | Visibility | ChildDeletion; // Union of tags that don't get reset on clones.
  ...
  const LowPriority = unstable_LowPriority;
  ...
  function diffHydratedProperties(domElement, tag, rawProps, parentNamespace, rootContainerElement, isConcurrentMode, shouldWarnDev) {
    ...
    for (const propKey in rawProps) {
      ...
      } else if (shouldWarnDev && false && // Convince Flow we've calculated it (it's DEV-only in this method.)
  ...
  function markUpdateLaneFromFiberToRoot(sourceFiber, update, lane) {
    ...
    while (parent !== null) {
      ...
      if (parent.tag === OffscreenComponent) {
        ...
        if (offscreenInstance !== null && !(offscreenInstance.visibility & OffscreenVisible)) {
          isHidden = true;
        }
      }
  ...
  const classComponentUpdater = {
    ...
    enqueueSetState(inst, payload, callback) {
      ...
      const root = enqueueUpdate$1(fiber, update, lane);
  ...
  const prevRenderLanesStackCursor = createCursor(NoLanes);
        ...
        case HostRoot:
          {
            ...
            const root = enqueueUpdate$1(provider, refreshUpdate, lane);
          ...
          if (nextDidTimeout !== prevDidTimeout) {
            ...
            if (nextDidTimeout) {
              ...
              if ((workInProgress.mode & ConcurrentMode) !== NoMode) {
                ...
                // more data to arrive.
                const isBadFallback = // It's bad to switch to a fallback if content is already visible
                current !== null && !prevDidTimeout && !isCurrentTreeHidden() || // Experimental: Some fallbacks are always bad
          ...
          if (isModernRoot) {
            ...
            if (newOffscreenSubtreeIsHidden) ; else {
              ...
              if (offscreenSubtreeWasHidden && !prevOffscreenSubtreeWasHidden) {
                ...
                recursivelyTraverseReappearLayoutEffects(finishedRoot, finishedWork, committedLanes, includeWorkInProgressEffects);
              } else {
                recursivelyTraverseLayoutEffects(finishedRoot, finishedWork, committedLanes);
              }
  ...
  function reappearLayoutEffects(finishedRoot, current, finishedWork, committedLanes, // This function visits both newly finished work and nodes that were re-used
  ...
  includeWorkInProgressEffects) {
    ...
    switch (finishedWork.tag) {
      ...
      case SimpleMemoComponent:
        {
          recursivelyTraverseReappearLayoutEffects(finishedRoot, finishedWork, committedLanes, includeWorkInProgressEffects); // TODO: Check flags & LayoutStatic
      ...
      case ClassComponent:
        {
          recursivelyTraverseReappearLayoutEffects(finishedRoot, finishedWork, committedLanes, includeWorkInProgressEffects); // TODO: Check for LayoutStatic flag
      ...
      case HostComponent:
        {
          recursivelyTraverseReappearLayoutEffects(finishedRoot, finishedWork, committedLanes, includeWorkInProgressEffects); // Renderers may schedule work to be done after host components are mounted
          // (eg DOM renderer may schedule auto-focus for inputs and form controls).
          // These effects should only be committed when components are first mounted,
          // aka when there is no current/alternate.
      ...
      case Profiler:
        {
          recursivelyTraverseReappearLayoutEffects(finishedRoot, finishedWork, committedLanes, includeWorkInProgressEffects); // TODO: Figure out how Profiler updates should work with Offscreen
      ...
      case SuspenseComponent:
        {
          recursivelyTraverseReappearLayoutEffects(finishedRoot, finishedWork, committedLanes, includeWorkInProgressEffects); // TODO: Figure out how Suspense hydration callbacks should work
          // with Offscreen.
  ...
  function recursivelyTraverseReappearLayoutEffects(finishedRoot, parentFiber, committedLanes, includeWorkInProgressEffects) {
    ...
    while (child !== null) {
      const current = child.alternate;
      reappearLayoutEffects(finishedRoot, current, child, committedLanes, childShouldIncludeWorkInProgressEffects);
      child = child.sibling;
    }
  }
      ...
      case OffscreenComponent:
        {
          ...
          if (isHidden && instance.visibility & OffscreenPassiveEffectsConnected && ( // For backwards compatibility, don't unmount when a tree suspends. In
  ...
  let workInProgressRootRenderLanes = NoLanes; // A contextual version of workInProgressRootRenderLanes. It is a superset of
  ...
  let renderLanes$1 = NoLanes; // Whether to root completed, errored, suspended, etc.
  ...
  function captureCommitPhaseErrorOnRoot(rootFiber, sourceFiber, error) {
    ...
    const root = enqueueUpdate$1(rootFiber, update, SyncLane);
    const eventTime = requestEventTime();
    ...
    while (fiber !== null) {
      if (fiber.tag === HostRoot) {
        ...
        if (typeof ctor.getDerivedStateFromError === 'function' || typeof instance.componentDidCatch === 'function' && !isAlreadyFailedLegacyErrorBoundary(instance)) {
          ...
          const root = enqueueUpdate$1(fiber, update, SyncLane);
          const eventTime = requestEventTime();
  ...
  callback, containerInfo, tag, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableError, transitionCallbacks) {
    const hydrate = true;
    const root = createFiberRoot(containerInfo, tag, hydrate, initialChildren, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableError); // TODO: Move this to FiberRoot constructor
  ...
  function updateContainer(element, container, parentComponent, callback) {
    ...
    const root = enqueueUpdate$1(current, update, lane);
  ...
  function createRoot(container, options) {
    ...
    const root = createContainer(container, ConcurrentRoot, null, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableError);
  ...
  function hydrateRoot(container, initialChildren, options) {
    ...
    const root = createHydrationContainer(initialChildren, null, container, ConcurrentRoot, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableError);
